...
def load_data(data_dir,spacing_file,is_train=True):
    ...
    data_list = []
    for brain_dir in glob.glob(os.path.join(data_dir,"soma_segmentation_training_images_withMultiSoma_removeSomaBadMultiSoma","*")):
        brain_id = os.path.split(brain_dir)[-1].split("_")[0]
        print("--> Loading for brain: {:s}".format(brain_id))
        spacing = spacing_dict[int(brain_id)]
        label_dir = os.path.join(data_dir,"soma_segmentation_training_segmentations_mirrorImage_withMultiSoma_removeSomaBadMultiSoma",brain_id+"_seg_watershed_SEU200")
        for imgfile in glob.glob(os.path.join(brain_dir,"*.tiff")):
            prefix = get_file_prefix(imgfile)
            if is_train:
                label_file = os.path.join(label_dir,f"seg_{prefix}.tiff")
            else:
                label_file = None
...
if __name__ == "__main__":
    data_dir = "/home/yjz/Projects/Auto_tracing/data/seu_mouse/crop_data/somaImage"
    spacing_file = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/scripts/AllbrainResolutionInfo.csv"
    output_dir = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet/data/task006_withMultiSomaOfflineAug_removeSomaBadMultiSoma"
    is_train = True
