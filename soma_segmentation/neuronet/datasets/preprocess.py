...
class GenericPreprocessor(object):
    ...
    if separate_z:
        ...
        for c in range(data.shape[0]):
            ...
            for slice_id in range(shape[z_axis]):
                reshapeed_data.append(resize(data[c,slice_id],new_shape[1:],order=order,mode="edge"))
            ...
            if shape[z_axis] != new_shape[z_axis]:
                ...
                map_rows,map_cols,map_dims = np.mgrid[:rows,:cols,:dims]
    ...
    def dataset_split(self,task_dir,val_rario=0.1,test_ratio=0.1,seed=1024,img_ext="npy",lab_ext="swc"):
        samples = []
        ...
        splits["test"] = splits[-num_test:]
        ## write to file
        with open(os,path.join(outputdir,"data_split.pkl"),"wb") as fp:
