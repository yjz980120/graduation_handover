...
def load_data(data_dir,spacing_file,is_train=True):
    ...
    data_list = []
    for brain_dir in glob.glob(os.path.join(data_dir,"img","*")):
...
if __name__ == "__main__":
    ...
    #data_dir = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet/exps/exp009/exp006_predictTest" ##注意这里用的complex soma 没有lab 所以img 和lab  都是同一个img，如果有lab的话要到上面改一下，指定lab的文件夹。或者由于是test阶段，可以直接指定is_train=False, False的时候就默认不读入lab 是None. ！！！ 还是不能指定is_train为False  因为后面evaluate的时候生成pred的同时 还要作evaluate 需要用到lab，后面可以自己改一个  改成单独用于infer的
    ...
    output_dir = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet/exps/exp012/exp012_predictMostComplexSoma"
    is_train = True
