...
def image_scale_4D(img, lab, spacing, shape, target_shape, mode, anti_aliasing, update_spacing):
    ...
    if lab is not None:
        ...
        lab = resize(lab, target_shape, order=0, mode=mode, anti_aliasing=anti_aliasing)## 这里不确定上面的 ： 为什么新的shape更小，也要指定插值方式？更小的话 不是直接下采样就可以了嘛？ 如果不是的话，那这里应该要跟上面的order保持一致。        
...
if __name__ == '__main__':
    ...
    imgfile = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet_reproduction/neuronet/data/task004_testSomaPreprocessNewGt/6235_5732_2862_img.npz"
    labfile = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet_reproduction/neuronet/data/task004_testSomaPreprocessNewGt/6235_5732_2862_lab.npz"
    ...
    if lab.max()<50:
        lab *= 255
    sitk.WriteImage(sitk.GetImageFromArray(np.squeeze(img).astype(np.uint8)),"img.tiff")
    sitk.WriteImage(sitk.GetImageFromArray(np.squeeze(lab).astype(np.uint8)),"lab.tiff")
