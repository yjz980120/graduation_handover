...
def image_scale_4D(img, lab, spacing, shape, target_shape, mode, anti_aliasing, update_spacing):
    ...
    if lab is not None:
        ...
        lab = resize(lab, target_shape, order=order, mode=mode, anti_aliasing=anti_aliasing)## 这里不确定上面的 ： 为什么新的shape更小，也要指定插值方式？更小的话 不是直接下采样就可以了嘛？ 如果不是的话，那这里应该要跟上面的order保持一致。        
...
class Compose(object):
    ...
    def __call__(self, img, lab=None, spacing=None):
        for t in self.transforms:
            img, lab, spacing = t(img, lab, spacing)
...
class ResizeToDividable(object):
    ...
    def __call__(self, img, lab=None, spacing=None):
        ...
        img, lab, spacing = image_scale_4D(img, lab, spacing, shape, target_shape, mode='edge', anti_aliasing=False, update_spacing=True)
...
class ConvertToFloat(object):
    ...
    def __call__(self, img, lab=None, spacing=None):
        ...
        if not lab.dtype.name.startswith('float'):
            lab = lab.astype(self.dtype)
        return img, lab, spacing
...
class ScaleToFixedSize(AbstractTransform):
    ...
    def __call__(self, img, lab=None, spacing=None):
        ...
        if np.random.random() < self.p:
            shape = np.array(img[0].shape)
            img, lab, spacing = image_scale_4D(img, lab, spacing, shape, self.target_shape, self.mode, self.anti_aliasing, self.update_spacing)
...
# RandomShift is an subset of composition of Crop and Padding, thus we do not need to implement it.(I(yjz) finished RandomShift for soma augmantation call this conveniently)
class RandomShift(AbstractTransform):
    def __init__(self, p=0.5,imgshape=None,shift_range=((1,3),(3,11),(3,11)),pad_value=None):
        super(RandomShift, self).__init__(p)
        self.shift_z_range = shift_range[0]
        self.shift_y_range = shift_range[1]
        self.shift_x_range = shift_range[2]
        self.imgshape = imgshape

    def __call__(self, img, lab=None, spacing=None):
        ## 下面这个也行，但是shift可能很小，因为本身crop就是随机的，结果pad得时候没有接着crop得位置来pad 而是随机pad，所以可能导致crop白费了：pad把crop得地方又给补上了
        """
        random_crop = RandomCrop(1.0,self.imagshape,crop_range=(0.92,1))
        img_c,lab_c,_ = random_crop(img,lab,spacing)
        target_shape = self.imgshape
        if pad_value == None:
            pad_value = img.min()
        else:
            pad_value = pad_value
        new_img = np.ones((img.shape[0],*target_shape),dtype=img.dtype) * pad_value
        sz = np.random.randint(0,target_shape[0]-img_c.shape[1])
        sy = np.random.randint(0,target_shape[1]-img_c.shape[2])
        sx = np.random.randint(0,target_shape[2]-img_c.shape[3])
        for c in range(len(new_img)):
            new_img[c][sz:sz+img_c.shape[1],sy:sy+img_c.shape[2],sx:sx+img_c.shape[3]] = img_c[c]
        if lab is not None:
            new_lab = np.zeros((*target_shape),dtype=lab.dtype)
            new_lab[sz:sz+lab_c.shape[0],sy:sy+lab_c.shape[1],sx:sx+lab_c.shape[2]] = lab_c
            return new_img,new_lab,spacing
        return img_c,lab_c,sapcing
        """
        ## 用这种方法可以避免上面的问题,但是更麻烦，后面可以用仿射变换
        shift_z = np.random.randint(self.shift_z_range[0],self.shift_z_range[1])
        shift_y = np.random.randint(self.shift_y_range[0],self.shift_y_range[1])
        shift_x = np.random.randint(self.shift_x_range[0],self.shift_x_range[1])

        random_z = np.random.choice([-1,1])
        random_y = np.random.choice([-1,1])
        random_x = np.random.choice([-1,1])
        pad_value = img.min()
        ## ！！！！！ 这里代码是从自己写的generate_multiSoma.py复制来的，那里有img_add,lab_add 这里为了方便就不改了，这里的img_add lab_add其实就是img lab
        img_add = copy.deepcopy(img)
        lab_add = copy.deepcopy(lab)
        ...
        if random_z>0 and random_y>0 and random_x>0:
            ...
            img_add_p[:,0:img_add.shape[1],0:img_add.shape[2],0:img_add.shape[3]] = img_add
            ...
            img_add = img_add[:,shift_z:,shift_y:,:-shift_x]
            img_add_p[:,0:img_add.shape[1],0:img_add.shape[2],(img_add_p.shape[3]-img_add.shape[3]):] = img_add
            lab_add = lab_add[shift_z:,shift_y:,:-shift_x]
            lab_add_p[0:lab_add.shape[0],0:lab_add.shape[1]:,(lab_add_p.shape[2]-lab_add.shape[2]):] = lab_add

        elif random_z>0 and random_y<0 and random_x>0:
            img_add = img_add[:,shift_z:,:-shift_y,shift_x:]
            img_add_p[:,0:img_add.shape[1],(img_add_p.shape[2]-img_add.shape[2]):,0:img_add.shape[3]] = img_add
            lab_add = lab_add[shift_z:,:-shift_y,shift_x:]
            lab_add_p[0:lab_add.shape[0],(lab_add_p.shape[1]-lab_add.shape[1]):,0:lab_add.shape[2]] = lab_add

        elif random_z>0 and random_y<0 and random_x<0:
            img_add = img_add[:,shift_z:,:-shift_y,:-shift_x]
            img_add_p[:,0:img_add.shape[1],(img_add_p.shape[2]-img_add.shape[2]):,(img_add_p.shape[3]-img_add.shape[3]):] = img_add
            lab_add = lab_add[shift_z:,:-shift_y,:-shift_x]
            lab_add_p[0:lab_add.shape[0],(lab_add_p.shape[1]-lab_add.shape[1]):,(lab_add_p.shape[2]-lab_add.shape[2]):] = lab_add

        elif random_z<0 and random_y>0 and random_x>0:
            img_add = img_add[:,:-shift_z,shift_y:,shift_x:]
            img_add_p[:,(img_add_p.shape[1]-img_add.shape[1]):,0:img_add.shape[2],0:img_add.shape[3]] = img_add
            lab_add = lab_add[:-shift_z,shift_y:,shift_x:]
            lab_add_p[(lab_add_p.shape[0]-lab_add.shape[0]):,0:lab_add.shape[1],0:lab_add.shape[2]] = lab_add

        elif random_z<0 and random_y>0 and random_x<0:
            img_add = img_add[:,:-shift_z,shift_y:,:-shift_x]
            img_add_p[:,(img_add_p.shape[1]-img_add.shape[1]):,0:img_add.shape[2],(img_add_p.shape[3]-img_add.shape[3]):] = img_add
            lab_add = lab_add[:-shift_z,shift_y:,:-shift_x]
            lab_add_p[(lab_add_p.shape[0]-lab_add.shape[0]):,0:lab_add.shape[1],(lab_add_p.shape[2]-lab_add.shape[2]):] = lab_add
        elif random_z<0 and random_y<0 and random_x>0:
            img_add = img_add[:,:-shift_z,:-shift_y,shift_x:]
            img_add_p[:,(img_add_p.shape[1]-img_add.shape[1]):,(img_add_p.shape[2]-img_add.shape[2]):,0:img_add.shape[3]] = img_add
            lab_add = lab_add[:-shift_z,:-shift_y,shift_x:]
            lab_add_p[(lab_add_p.shape[0]-lab_add.shape[0]):,(lab_add_p.shape[1]-lab_add.shape[1]):,0:lab_add.shape[2]] = lab_add

        elif random_z<0 and random_y<0 and random_x<0:
            img_add = img_add[:,:-shift_z,:-shift_y,:-shift_x]
            img_add_p[:,(img_add_p.shape[1]-img_add.shape[1]):,(img_add_p.shape[2]-img_add.shape[2]):,(img_add_p.shape[3]-img_add.shape[3]):] = img_add
            lab_add = lab_add[:-shift_z,:-shift_y,:-shift_x]
            lab_add_p[(lab_add_p.shape[0]-lab_add.shape[0]):,(lab_add_p.shape[1]-lab_add.shape[1]):,(lab_add_p.shape[2]-lab_add.shape[2]):] = lab_add        
        
        return img_add_p,lab_add_p,spacing
...
class InstanceAugmentation(object):
    ...
    def __call__(self, img, lab=None, spacing=None):
        return self.augment(img, lab, spacing)


if __name__ == '__main__':
    import pickle
    import SimpleITK as sitk
    split_file = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet_reproduction/neuronet/data/task002_testSomaPreprocess/data_splits.pkl"
    phase="train"
    #import ipdb;ipdb.set_trace()
    ...
    #data_list = load_data_list(split_file,phase)
    imgfile = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet/data/task006_withMultiSomaOfflineAug_removeSomaBadMultiSoma/ImgSoma_17302_00020-x_14992.3_y_21970.3_z_4344.8_img.npz"
    labfile = "/home/yjz/Projects/Auto_tracing/neuronet_forSoma/neuronet/data/task006_withMultiSomaOfflineAug_removeSomaBadMultiSoma/ImgSoma_17302_00020-x_14992.3_y_21970.3_z_4344.8_lab.npz"
    spacing = np.array([1.  , 0.23, 0.23])
    imgshape = (128,160,160)
    #imgfile,labfile,spacing = data_list[0]
    img = np.load(imgfile)["data"]
    if img.ndim == 3:
        img = img[None]
    lab = np.load(labfile)["data"]
    augment = InstanceAugmentation(p=0.2,imgshape=(128,160,160),phase=phase)
    #import ipdb;ipdb.set_trace()
    img,lab,_ = augment(img,lab,spacing)
    img = unnormalize_normal(img)
    if img.max()<50:
        img *= 255
    if lab.max()<50:
        lab *= 255
    sitk.WriteImage(sitk.GetImageFromArray(np.squeeze(img).astype(np.uint8)),"img-1-11.tiff")
    sitk.WriteImage(sitk.GetImageFromArray(np.squeeze(lab).astype(np.uint8)),"lab-1-11.tiff")
