...
template<class T> bool happ(vector<MyMarker*> &inswc, vector<MyMarker*> & outswc, T * inimg1d, long sz0, long sz1, long sz2, double bkg_thresh = 10.0, double length_thresh = 2.0, double SR_ratio = 1.0/9.0, bool is_leaf_prune = true, bool is_smooth = true)
{
    ...
    //swc2topo_segs(inswc, topo_segs, leaf_markers, INTENSITY_DISTANCE_METHOD, inimg1d, sz0, sz1, sz2);
    swc2topo_segs(inswc, topo_segs, INTENSITY_DISTANCE_METHOD, inimg1d, sz0, sz1, sz2);
    // yufeng: the segment length should not be too short
	vector<HierarchySegment*> filter_segs;
    for(V3DLONG i = 0; i < topo_segs.size(); i++)
	{
		if(topo_segs[i]->length >= length_thresh) filter_segs.push_back(topo_segs[i]);
    ...
    // use a more aggresssive threshold for final reconstruction
    double imgAve, imgStd;
    long sz = sz0 * sz1 * sz2;
    mean_and_std(inimg1d, sz, imgAve, imgStd);
    double td= (imgStd<10)? 10: imgStd;

	if(1) // dark nodes pruning
	{
        double dark_thresh = imgAve + 1.0*td > bkg_thresh ? imgAve + 1.0*td : bkg_thresh;
		...
		while(dark_num_pruned > 0)
		{
            ...
            for(V3DLONG i = 0; i < filter_segs.size(); i++)
			{
				...
				if(inimg1d[leaf_marker->ind(sz0, sz01)] <= dark_thresh)
				{
					seg->leaf_marker = leaf_marker->parent;
                    //yufeng: update the seg length
                    if (leaf_marker->parent) {
                        double tmp_dst = (INTENSITY_DISTANCE_METHOD == 0) ? inimg1d[leaf_marker->parent->ind(sz0, sz01)]/255.0 : dist(*leaf_marker, *(leaf_marker->parent));
                        seg->length = seg->length - tmp_dst;
                    }
        ...
        // yufeng: re-filtering with length of segs
        vector<HierarchySegment*> tmp_segs;
        for(int i = 0; i < filter_segs.size(); i++)
        {
            HierarchySegment * seg = filter_segs[i];
            if (seg->length >= length_thresh) tmp_segs.push_back(seg);
        }
        cout<<"\t"<< filter_segs.size() - tmp_segs.size() << " short segments after dark nodes pruning are deleted"<<endl;
        filter_segs = tmp_segs;
	}

	if(1) // dark segment pruning
	{
        double dark_thresh = imgAve + 0.7*td > bkg_thresh ? imgAve + 0.7*td : bkg_thresh;
        ...
        for(V3DLONG i = 0; i < filter_segs.size(); i++)
		{
			...
			while(true)
			{
				...
				sum_int += intensity;
				tol_num++;
				if(intensity <= dark_thresh) dark_num++;
				if(p == root_marker) break;
				p = p->parent;
			}
            if((sum_int/tol_num <= dark_thresh ||
               dark_num/tol_num >= 0.2) && (seg->length < 120))
    ...
    for(V3DLONG i = 0; i < filter_segs.size(); i++)
	{
		double dst = filter_segs[i]->length;    // yufeng: warnining: the length should be updated!
