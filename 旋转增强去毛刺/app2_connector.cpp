...
#include "rotation_augmentation.h"

bool standardize(unsigned char* data, int num, double median_of_mean = 5.312, double median_of_std = 2.267, double eps=1e-7) {
    double mean = 0, std = 0;
    for (int i=0; i< num; i++){
        //mean += (double)data[i];
        mean += data[i];
...
Swc_Swcfile prune_swc(PARA_APP2 &p, vector<MyMarker*> & inswc, unsigned char * indata1d, V3DLONG* in_sz, qint64 etime1, QElapsedTimer timer2, vector<MyMarker> inmarkers, double dfactor_xy, double dfactor_z, string tmpstr, QString qtstr, list<string> infostring, V3DPluginCallback2 &callback){
    vector<MyMarker*> outswc;
    QString outswc_file;
    //unsigned char t2 = indata1d[0];
    ...
    if(1)
    {
        ...
        if(!p.outswc_file.isEmpty())
            ...
            outswc_file = QString(p.p4dImage->getFileName()) + rootposstr + "_app3.swc";
...
//        callback.getView3DControl(curwin)->updateWithTriView();
    ...
    unordered_set<MyMarker*> s;
    for (auto leaf : outswc){
        if (!s.count(leaf)) s.insert(leaf);
    }

    vector<MyMarker*> temp_outswc;
    for (auto leaf : s){
        temp_outswc.push_back(leaf);
    }


    Swc_Swcfile res;
    res.outswc = temp_outswc;
    res.outswc_file = outswc_file;
    return res;
}
...
bool proc_app2(V3DPluginCallback2 &callback, PARA_APP2 &p, const QString & versionStr)
{
  //  bool b_menu = true;
    ...
    tmpstr =  qPrintable( qtstr.setNum(p.bkg_thresh_k).prepend("#bkg_thresh_k = ") ); infostring.push_back(tmpstr);
    tmpstr =  qPrintable( qtstr.setNum(p.rotation).prepend("#rotation = ") ); infostring.push_back(tmpstr);
    tmpstr =  qPrintable( qtstr.setNum(p.ini_swc).prepend("#ini_swc = ") ); infostring.push_back(tmpstr);
    ...
    int temp_num = p4dImageNew->getXDim() * p4dImageNew->getYDim() * p4dImageNew->getZDim() * p4dImageNew->getCDim();
    cout<<"pixel_num:"<<temp_num<<endl;
    unsigned char * data_temp;
    data_temp = p4dImageNew->getRawData();
    //cout<<"data1d[0]:"<<(int)data_temp[0]<<endl;
    //cout<<"data1d[-1]"<<(int)data_temp[temp_num-1]<<endl;
    standardize(data_temp, temp_num);
    //cout<<"after:data1d[0]:"<<(int)data_temp[0]<<endl;
    //cout<<"after:data1d[-1]"<<(int)data_temp[temp_num-1]<<endl;
    //cout<<"true:data1d[0]:"<<(int)(p4dImageNew->getRawData())[0]<<endl;
    //cout<<"true:data1d[-1]:"<<(int)(p4dImageNew->getRawData())[temp_num-1]<<endl;

    //p4dImageNew->setNewRawDataPointer(data_temp);
    //cout<<"after:true:data1d[0]:"<<(int)(p4dImageNew->getRawData())[0]<<endl;
    //cout<<"after:true:data1d[-1]:"<<(int)(p4dImageNew->getRawData())[temp_num-1]<<endl;
    unsigned char * indata1d = p4dImageNew->getRawDataAtChannel(0);
    unsigned char * temp_t = p4dImageNew->getRawData();
    double mean=0, std=0;
    ...
    cout<<"after:mean:"<<mean<<endl;
    for (int i=0; i< temp_num; i++){
        std += (temp_t[i] - mean) * (temp_t[i] - mean);
    }
    std = sqrt(std/temp_num);
    cout<<"after:std:"<<std<<endl;
    V3DLONG in_sz[4] = {p4dImageNew->getXDim(), p4dImageNew->getYDim(), p4dImageNew->getZDim(), 1};
    V3DLONG in_sz_forRestoreRotatedSwc[4];
    for (int i=0;i<4;i++) in_sz_forRestoreRotatedSwc[i] = in_sz[i];
...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
            ...
            //if (p.bkg_thresh_k < 0)
            p.bkg_thresh = imgAve +p.bkg_thresh_k*td ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5; //20170523, PHC
            //else p.bkg_thresh = imgAve +p.bkg_thresh_k*td ;
        }
        else
            p.bkg_thresh = 0;
    ...
    /*
    if (p.rotation){
        vector<MyMarker *> rotated_outtree;
        cout<<"Use ratation augmentation"<<endl;
        unsigned char * rotated_indata1d = rotation_image(indata1d, in_sz);
    }*/

    vector<MyMarker *> rotated_outtree;
    unsigned char * rotated_indata1d;
    if (p.rotation){
        cout<<"Use ratation augmentation"<<endl;
        rotated_indata1d = rotation_image(indata1d, in_sz);
        //char outimg_file[] = {'.','/','t','.','t','i','f','f'};
        string out_file = "/home/yjz/Project/Auto-tracing/data/seu_mouse/crop_data/dendriteImageMaxR_AllData/tiff/17302/new_t.tiff";
        char outimg_file[out_file.size()];
        for (int i=0; i<out_file.size();i++)  outimg_file[i] = out_file[i];
        //V3DLONG t_sz[4] = {1024,1024,512,1};
        simple_saveimage_wrapper(callback, outimg_file, rotated_indata1d, in_sz, datatype);
    }
...
//                if (p.rotation){
//                    fastmarching_dt_XY(rotated_indata1d, rotated_phi, in_sz[0], in_sz[1], in_sz[2],p.cnn_type, p.bkg_thresh);
//                }
                break;
...
//                if (p.rotation){
//                    fastmarching_dt_XY((short int*)rotated_indata1d, rotated_phi, in_sz[0], in_sz[1], in_sz[2],p.cnn_type, p.bkg_thresh);
//                }
                break;
        default:
    ...
    else if(inmarkers.size() == 1)
    {
        ...
        if(p.is_gsdt)
        {
            if(phi == 0)
            {
                ...
                switch(datatype)
                {
                    case V3D_UINT8:
                        ...
                        if (p.rotation){
                            fastmarching_dt(rotated_indata1d, rotated_phi, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh);
                        }
                        break;
                    case V3D_UINT16:  //this is no longer needed, as the data type has been converted above
                        fastmarching_dt((short int *)indata1d, phi, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh);
                        if (p.rotation){
                            fastmarching_dt((short int *)rotated_indata1d, rotated_phi, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh);
                        }
                        break;
                    default:
                        v3d_msg("Unsupported data type");
            ...
            if (p.rotation){
                fastmarching_tree(inmarkers[0], rotated_phi, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh, p.is_break_accept);
            }
        }
        else
        {
            switch(datatype)
            {
                case V3D_UINT8:
                    ...
                    if (p.rotation){
                        fastmarching_tree(inmarkers[0], rotated_indata1d, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh, p.is_break_accept);
                    }
                    break;
                case V3D_UINT16: //this is no longer needed, as the data type has been converted above
                    v3d_msg("16bit", 0);
                    ...
                    if (p.rotation){
                        fastmarching_tree(inmarkers[0], (short int*)rotated_indata1d, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh, p.is_break_accept);
                    }
                    break;
                default:
                    v3d_msg("Unsupported data type");
    ...
    else
    {
        ...
        if(p.is_gsdt)
        {
            ...
            if (p.rotation){
                fastmarching_tree(inmarkers[0], target, rotated_phi, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type);
            }
        }
        else
        {
            switch(datatype)
            {
                case V3D_UINT8:
                    ...
                    if (p.rotation){
                        fastmarching_tree(inmarkers[0], target, rotated_indata1d, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type);
                    }
                    break;
                case V3D_UINT16:
                    fastmarching_tree(inmarkers[0], target, (short int*) indata1d, outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type);
                    if (p.rotation){
                        fastmarching_tree(inmarkers[0], target, (short int*) rotated_indata1d, rotated_outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type);
                    }
                    break;
            }
        }
    ...
    /*
    if (p.rotation){
        rotation_swc(rotated_outtree, in_sz);
        consensus(outtree,rotated_outtree,in_sz);
    }*/
    ...
    vector<MyMarker*> & rotated_inswc = rotated_outtree;
    //if (p.rotation)
        //rotated_inswc = rotated_outtree;
    ...
    if (p.rotation){
        if (1)
        {
            ...
            vector<MyMarker*> tmpswc;
            for (tmpi=0; tmpi<rotated_inswc.size(); tmpi++)
            {
                MyMarker * curp = new MyMarker(*(rotated_inswc[tmpi]));
                tmpswc.push_back(curp);

                if (dfactor_xy>1) rotated_inswc[tmpi]->x *= dfactor_xy;
                rotated_inswc[tmpi]->x += (p.xc0);
                if (dfactor_xy>1) rotated_inswc[tmpi]->x += dfactor_xy/2;

                if (dfactor_xy>1) rotated_inswc[tmpi]->y *= dfactor_xy;
                rotated_inswc[tmpi]->y += (p.yc0);
                if (dfactor_xy>1) rotated_inswc[tmpi]->y += dfactor_xy/2;

                if (dfactor_z>1) rotated_inswc[tmpi]->z *= dfactor_z;
                rotated_inswc[tmpi]->z += (p.zc0);
                if (dfactor_z>1)  rotated_inswc[tmpi]->z += dfactor_z/2;
            }
            ...
            for (tmpi=0; tmpi<rotated_inswc.size(); tmpi++)
            {
                rotated_inswc[tmpi]->x = tmpswc[tmpi]->x;
                rotated_inswc[tmpi]->y = tmpswc[tmpi]->y;
                rotated_inswc[tmpi]->z = tmpswc[tmpi]->z;
            }
    ...
    vector<MyMarker*> outswc;
    QString outswc_file;
    //if (p4dImageNew) {delete p4dImageNew; p4dImageNew=0;}
    Swc_Swcfile outswc_andFile = prune_swc(p, inswc, indata1d, in_sz, etime1, timer2, inmarkers, dfactor_xy, dfactor_z, tmpstr, qtstr, infostring, callback);
    outswc = outswc_andFile.outswc;
    outswc_file = outswc_andFile.outswc_file;


    vector<MyMarker*> rotated_outswc;
    Swc_Swcfile rotated_outswc_andFile;
    if (p.rotation){

        Swc_Swcfile rotated_outswc_andFile = prune_swc(p, rotated_inswc, rotated_indata1d, in_sz, etime1, timer2, inmarkers, dfactor_xy, dfactor_z, tmpstr, qtstr, infostring, callback);
        rotated_outswc = rotated_outswc_andFile.outswc;

        rotation_swc(rotated_outswc, in_sz_forRestoreRotatedSwc);
        //saveSWC_file(rotated_outswc_andFile.outswc_file.toStdString(), rotated_outswc, infostring);
        //vector<MyMarker *> pruned_tree;
        outswc = consensus(outswc,rotated_outswc,in_sz_forRestoreRotatedSwc);
        saveSWC_file(rotated_outswc_andFile.outswc_file.toStdString(), outswc, infostring);
    }
    ...
    if(outswc.size()>1)
        ...
        //call sort_swc function
            ...
            arg.type = "random";std::vector<char*> arg_resample_para; arg_resample_para.push_back("1");arg.p = (void *) & arg_resample_para; input_resample << arg;
    ...
    if (p.rotation){
    if(rotated_phi){delete [] rotated_phi; rotated_phi = 0;}
    for(V3DLONG i = 0; i < rotated_outtree.size(); i++) delete rotated_outtree[i];
    rotated_outtree.clear();
    }
    if (b_dofunc)
    {
...
bool PARA_APP2::fetch_para_commandline(const V3DPluginArgList &input, V3DPluginArgList &output, V3DPluginCallback2 &callback, QWidget *parent)
{
  ...
  //  outswc_file = outfiles.empty() ? inimg_file + "_app2.swc" : outfiles[0];
    ...
    bkg_thresh_k = (paras.size() >= k+1) ? atof(paras[k]) : bkg_thresh_k; k++;//-1 (0.5)
    rotation = (paras.size() >= k+1) ? atoi(paras[k]) : rotation; k++;//false
    ini_swc = (paras.size() >= k+1) ? atoi(paras[k]) : ini_swc; k++;//true
