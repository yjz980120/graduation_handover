...
bool prune_swc(PARA_APP2 &p, vector<MyMarker*> & inswc, unsigned char * indata1d, V3DLONG* in_sz, qint64 etime1, QElapsedTimer timer2, Image4DSimple * p4dImageNew,vector<MyMarker> inmarkers, double dfactor_xy, double dfactor_z, string tmpstr, QString qtstr, list<string> infostring, V3DPluginCallback2 &callback){
...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
    ...
    //outswc = prune_swc(p, inswc, indata1d, in_sz, etime1, etime2); 
    prune_swc(p, inswc, indata1d, in_sz, etime1, timer2, p4dImageNew, inmarkers, dfactor_xy, dfactor_z, tmpstr, qtstr, infostring, callback);
