...
def find_breakTip2Pairs(swc_file, morph, dist_thresh, line_length, angle_thresh):
    mfa = MultiFurcationAnalyzer(app2_swc_file, dist_thresh, line_length, soma_radius=soma_threshold, downsampling=False, morph_existed=True, morph=morph)
...
def calc_segment_radius(morph, segment):
    segment_radius = 0.0
    for idx in segment:
        segment_radius += morph.pos_dict[idx][5]
    if len(segment) != 0:
        segment_radius /= len(segment)

    return segment_radius

def calc_segment_intensity(img, segment):
    segment_intensity = 0.0
    img = img[:,::-1,:]
    for idx in segment:
        #import ipdb; ipdb.set_trace()
        x = round(morph.pos_dict[idx][2])
        y = round(morph.pos_dict[idx][3])
        z = round(morph.pos_dict[idx][4])
        
        segment_intensity += img[z][y][x]
    if len(segment) != 0:
        segment_intensity /= len(segment)
    
    return segment_intensity
        

def overall_judge_radius_andIntensity_decreasing(cut_morph, tiff_file, count):
    ori_img = sitk.GetArrayFromImage(sitk.ReadImage(tiff_file))
    all_segments = []
    for cur_tip in cut_morph.tips:
        all_segments = []
        cur_node = cur_tip
        cur_count = 0
        cur_segment = []
        while cur_node != -1 and cur_node != -2:
            if cur_count < count:
                if cur_node in cut_morph.child_dict:
                    if len(cut_morph.child_dict[cur_node]) == 1:
                        cur_segment.append(cur_node)
                else:
                    cur_segment.append(cur_node)
                cur_count += 1
                cur_node = cut_morph.pos_dict[cur_node][6]
        ...
        if len(all_segments) > 1:
            for i in range(1, len(all_segments)):
                segment_s = all_segments[i-1]
                segment_p = all_segments[i]
                #if tiff_file == "/media/yjz/4A0263A70263972B/SEU_Data/seu_mouse/crop_data/dendriteImageMaxR/tiff/18454/14528_10688_3118.tiff":
                    #if 17153 in segment_s or 17153 in segment_p:
                        #import ipdb; ipdb.set_trace()
                segment_s_radius = calc_segment_radius(cut_morph, segment_s)
                segment_p_radius = calc_segment_radius(cut_morph, segment_p)
                segment_s_intensity = calc_segment_intensity(ori_img, segment_s)
                segment_p_intensity = calc_segment_intensity(ori_img, segment_p)
                    
                #import ipdb; ipdb.set_trace()
                if segment_s_radius > segment_p_radius or segment_s_intensity > segment_p_intensity:
                #if segment_s_radius > segment_p_radius:
                #if segment_s_intensity > segment_p_intensity:
                    #import ipdb; ipdb.set_trace()
                    temp_point = list(cut_morph.pos_dict[segment_s[-1]])
                    #temp_x = temp_point[2]
                    #temp_y = temp_point[3]
                    #temp_z = temp_point[4]
                    temp_cor = np.array(temp_point[2:5])
                    soma_cor = np.array(cut_morph.pos_dict[cut_morph.idx_soma][2:5])
                    temp_dist = np.linalg.norm(temp_cor - soma_cor)
                    if temp_dist > ori_img.shape[2]/3:
                        temp_point[6] = -2
                        cut_morph.pos_dict[segment_s[-1]] = tuple(temp_point)
...
if __name__ == '__main__':
    ...
    app2_swc_dir = "/home/yjz/Project/Auto-tracing/data/seu_mouse/crop_data/dendriteImageMaxR_AllData/app2_upsample3_toyset_rotationAug_sorted"
    post_swc_dir = "/home/yjz/Project/Auto-tracing/crossing/Myself_newMethod/data/post_processing/add_moreFeatures/maxR_toyset_angleThresh10_withoutMoreFea_withoutClaw_addSomaDetCutLargerSomaNum10_addOverallFeatures_RadiusOrInten"
    ...
    small_soma_swc = 0
    large_soma_swc = 0
    ## 只有一个soma的时候是否进行prune, True就是仍然进行处理
    one_soma = True
    ## 要不要从全局角度考虑增加radius和intensity特征(从tip末端点开始dfs就行，不符合递减假设的就断开) 
    overall_judge_radius_andIntensity=True
    overall_count = 5 ##多少个点组成一个segment
    ...
    for brain in glob.glob(os.path.join(app2_swc_dir,"[1-9]*[1-9]")):
        ...
        for app2_swc_file in glob.glob(os.path.join(brain,'*.swc')):
            ...
            if soma_detect:
                ...
                if len(all_soma) > 1 and len(all_soma) <= soma_cut_num:
                    small_soma_swc += 1 
            ...
            if overall_judge_radius_andIntensity:
                 
                print("Judge Radius Intensity Cut:")
                tiff_file = os.path.join(tiff_dir, brain_id, swc_prefix+".tiff")
                #if tiff_file == "/media/yjz/4A0263A70263972B/SEU_Data/seu_mouse/crop_data/dendriteImageMaxR/tiff/18454/14528_10688_3118.tiff":
                    #import ipdb; ipdb.set_trace()
                #if soma_detection_cut and len(all_soma) > 1:
                cut_morph = overall_judge_radius_andIntensity_decreasing(cut_morph, tiff_file, overall_count)
                #else:
                    #cut_morph = overall_judge_radius_andIntensity_decreasing(morph, tiff_file, overall_count)
                pruned_pos_dict = prune_notConnenctToSoma(app2_swc_file, cut_morph)
                #cut_morph.pos_dict = pruned_pos_dict 
                pruned_tree = pos_dict_toTree(pruned_pos_dict)
                cut_morph = morphology.Morphology(pruned_tree, p_soma=-1)
            ...
            if len(app2_stps) == 0 and len(app2_multifurcation_cor) == 0:
                ...
                if not (soma_detection_cut or overall_judge_radius_andIntensity):
                    ## 注意即使不存在多分叉，但是由于前面还进行了多soma检测与cut或者从全局角度根据radius和intensity特征进行cut,那么crossing这里可以不继续执行，但是要把前面两步的cut结果保存
                    print("Finished!")
                    print("\n")
                    continue
                else:
                    ...
                    #pruned_tree = pos_dict_toTree(pruned_pos_dict)
                    ##直接保存上面处理过的pruned_tree就行（因为在上面的pruned_tree之后也没有做过别的什么Prune操作）
                    if allData:
            ...
            #import ipdb; ipdb.set_trace()
            #if swc_prefix == "10426_10338_6106":
                #import ipdb; ipdb.set_trace()
            if soma_detection_cut and len(all_soma) > 1:
                break_tip2_pairs = find_breakTip2Pairs_somaDetCut(app2_swc_file, post_morph, dist_thresh, line_length, ang_thresh)
            else:
                break_tip2_pairs = find_breakTip2Pairs(app2_swc_file, post_morph, dist_thresh, line_length, ang_thresh)
            ...
            for i, idx in enumerate(include_crossing_likely_pairs_idx):
                if type(idx) == tuple:
                    ...
                    if local_judge_radius_andIntensity:
                        tiff_file = os.path.join(tiff_dir, brain_id, swc_prefix+".tiff")
                        decreasing_s1, decreasing_s2, decreasing_s3 = local_judge_radius_andIntensity_decreasing(post_morph, tiff_file, nearest_p, nearest_s1, nearest_s2, nearest_s3, count=local_count)
                    if local_judge_radius_andIntensity:
