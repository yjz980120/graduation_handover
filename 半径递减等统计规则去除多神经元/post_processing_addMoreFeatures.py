...
def judge_radius_andIntensity_decreasing(morph, tiff_file, nearest_p, nearest_s1, nearest_s2, nearest_s3, count=5):
...
if __name__ == '__main__':
    ...
    post_swc_dir = "/home/yjz/Project/Auto-tracing/crossing/Myself_newMethod/data/post_processing/add_moreFeatures/maxR_toyset_angleThresh60"
    ...
    joint_max_angle_thresh_2breakTip2 = 60#90
    joint_max_angle_thresh = 60#90 #常用120#140
    ##要不要增加radius和intensity特征
    judge_radius_andIntensity = True
        ...
        for app2_swc_file in glob.glob(os.path.join(brain,'*.swc')):
            ...
            for i, idx in enumerate(include_crossing_likely_pairs_idx):
                if type(idx) == tuple:
                    ...
                    decreasing_s2 = True
                    decreasing_s3 = True
                    #judge_radius_andIntensity = False
                    #import ipdb; ipdb.set_trace()
                    if judge_radius_andIntensity:
                        tiff_file = os.path.join(tiff_dir, brain_id, swc_prefix+".tiff")
                        decreasing_s1, decreasing_s2, decreasing_s3 = judge_radius_andIntensity_decreasing(post_morph, tiff_file, nearest_p, nearest_s1, nearest_s2, nearest_s3)
                    if judge_radius_andIntensity:
