...
class RandomSaturation(AbstractTransform):
    ...
    def __call__(self, img, tree=None, spacing=None):
        if np.random.random() < self.p:
            ...
            ori_img *= sf
            img = np.concatenate((ori_img,path_img),axis=0)
...
class RandomBrightness(AbstractTransform):
    ...
    def __call__(self, img, tree=None, spacing=None):
        if np.random.random() < self.p:
            ori_img = img[0]
            path_img = img[1:]
            img_flat = img.reshape((ori_img.shape[0],-1))
            ...
            ori_img += dmm.reshape((mm.shape[0],1,1,1))
...
class RandomGaussianNoise(AbstractTransform):
    ...
    def __call__(self, img, tree=None, spacing=None):
        if np.random.random() < self.p:
            ...
            ## 注意只用对原图进行这个增强，所以只用对现在读进来的图的第一个channel 进行增强即可
            ori_img = img[0]
            path_img = img[1:]
            img_flat = ori_img.reshape((ori_img.shape[0],-1))
            mm = img_flat.max(axis=1) - img_flat.min(axis=1)
            
            keep_ratio = np.random.uniform(0, self.max_ratio)
            mask = np.random.random(size=ori_img.shape) < keep_ratio
            noise = np.random.normal(0, var, size=ori_img.shape) * mm.reshape((-1,1,1,1)) * mask.astype(np.float32)
    ...
    def __call__(self, img, tree=None, spacing=None):
        ...
        if np.random.random() < self.p:
            ...
            for c in range(ori_img.shape[0]):
                ori_img[c] = gaussian_filter(ori_img[c], sigma=sigmas)
            img = np.concatenate((ori_img,path_img),axis=0)
        return img, tree, spacing
    ...
    def __init__(self, p=0.2, imgshape=(256,512,512), phase='train', divid=2**5):
        if phase == 'train':
            ...
            self.augment = Compose([
                ...
                #RandomResample(p=p),
                RandomMirror(p=p),# 这个和上一个RandomResample都先不加，因为上面几个都是只需要改原始img就行，但是这两个应该还要对应把其他通道的path图也改了
            ...
            self.augment = Compose([
                ...
                #GammaTransform(gamma=0.4, trunc_thresh=0.216,retain_stats=True),
            ])
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    ...
    img = np.random.random(size=(64,4,32,32,32))
    phase = "train"
    spacing = (0.23,0.23,0.23)
    #import ipdb; ipdb.set_trace()
    augment = InstanceAugmentation(p=1.0, phase=phase)
    augment(img,spacing=spacing)
