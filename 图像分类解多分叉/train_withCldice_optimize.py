...
from neuronet.loss.cldice_loss import Soft_cldice,Soft_dice_cldice
...
def get_forward(img_d, lab_d, crit_ce, crit_dice, crit_cldice,model):
    logits = model(img_d)
    ...
    loss_ce_items, loss_dice_items ,loss_cldice_items = [], [], []
    ...
    for i in range(len(logits)):
        ...
        loss_cldice = crit_cldice(probs * loss_weights_unsq, lab_d.float() * loss_weights)
        loss_ce_items.append(loss_ce.item())
        loss_dice_items.append(loss_dice.item())
        loss_cldice_items.append(loss_cldice.item())
        if i == 0:
            loss = (loss_ce + loss_dice + loss_cldice) * weights[i]
        else:
            loss += (loss_ce + loss_dice + loss_cldice) * weights[i]
    return loss_ce_items, loss_dice_items, loss_cldice_items, loss, logits[0]

def get_forward_eval(img_d, lab_d, crit_ce, crit_dice, crit_cldice, model):
    if args.amp:
        with autocast():
            with torch.no_grad():
                loss_ces, loss_dices, loss_cldices, loss, logits = get_forward(img_d, lab_d, crit_ce, crit_dice, crit_cldice, model)
    else:
        with torch.no_grad():
            loss_ces, loss_dices, loss_cldices, loss, logits = get_forward(img_d, lab_d, crit_ce, crit_dice, crit_cldice, model)
    return loss_ces, loss_dices, loss_cldices, loss, logits


def validate(model, val_loader, crit_ce, crit_dice, crit_cldice, epoch, debug=True, num_image_save=10, phase='val'):
    model.eval()
    num_saved = 0
    if num_image_save == -1:
        num_image_save = 9999
    ...
    for img,lab,imgfiles,swcfiles in val_loader:
        ...
        if phase == 'val':
            loss_ces, loss_dices, loss_cldices, loss, logits = get_forward_eval(img_d, lab_d, crit_ce, crit_dice, crit_cldice, model)
        elif phase == 'test':
            ...
            for ie in range(n_ens):
                ...
                logits_list = []
                loss_ces, loss_dices, loss_cldices, loss = [], [], [], 0
                for i in range(len(crops)):
                    loss_ces_i, loss_dices_i, loss_cldices_i, loss_i, logits_i = get_forward_eval(crops[i][None], lab_crops[i][None], crit_ce, crit_dice, crit_cldice, model)
                    logits_list.append(logits_i[0])
                    loss_ces.append(loss_ces_i)
                    loss_dices.append(loss_dices_i)
                    loss_cldices.append(loss_dices_i)
                    loss += loss_i
                ...
                loss_cldices = np.array(loss_cldices).mean(axis=0)
                loss /= ncrop
        ...
        losses.append([loss_ces[0], loss_dices[0], loss_cldices[0], loss.item()])
...
def evaluate(model, optimizer, crit_ce, crit_dice, crit_cldice, imgshape):
    phase = 'test'
    val_loader, val_iter = load_dataset(phase, imgshape)
    loss_ce, loss_dice, loss_cldice, loss = validate(model, val_loader, crit_ce, crit_dice, crit_cldice, epoch=0, debug=True, num_image_save=-1, phase=phase)
    ddp_print(f'Average loss_ce and loss_dice and loss_cldice: {loss_ce:.5f} {loss_dice:.5f} {loss_cldice:.5f}')

def train(model, optimizer, crit_ce, crit_dice, crit_cldice, imgshape):
    # dataset preparing
    ...
    for epoch in range(args.max_epochs):
        ...
        for it in range(args.step_per_epoch):
            ...
            avg_loss_cldice += loss_cldices[0]

            # train statistics for bebug afterward
            if it % args.print_frequency == 0:
                ddp_print(f'[{epoch}/{it}] loss_ce={loss_ces[0]:.5f}, loss_dice={loss_dices[0]:.5f}, loss_cldice={loss_cldices[0]:.5f} ,time: {time.time() - t0:.4f}s')
        ...
        avg_loss_cldice /= args.step_per_epoch

        # do validation
        if epoch % args.test_frequency == 0:
            ddp_print('Evaluate on val set')
            val_loss_ce, val_loss_dice, val_loss_cldice, val_loss = validate(model, val_loader, crit_ce, crit_dice, crit_cldice, epoch, debug=debug, phase='val')
            model.train()   # back to train phase
            ddp_print(f'[Val{epoch}] average ce loss and dice loss and cldice loss are {val_loss_ce:.5f}, {val_loss_dice:.5f}, {val_loss_cldice:.5f}')
            # save the model
            if args.is_master:
                # save current model
                torch.save(model, os.path.join(args.save_folder, 'final_model.pt'))

                if val_loss_dice + val_loss_cldice < best_loss_dice + best_loss_cldice:
                    best_loss_dice = val_loss_dice
                    best_loss_cldice = val_loss_cldice
                    print(f'Saving the model at epoch {epoch} with dice loss and cldice loss {best_loss_dice:.4f} {best_loss_cldice:.4f}')
...
def main():
    ...
    crit_cldice = Soft_cldice(smooth=1e-5, input_logits=False).to(args.device)
    ...
    if args.evaluation:
        evaluate(model, optimizer, crit_ce, crit_dice, crit_cldice, args.imgshape)
    else:
        train(model, optimizer, crit_ce, crit_dice, crit_cldice, args.imgshape)
