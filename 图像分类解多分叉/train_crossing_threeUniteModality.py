...
class CrossingDataset(tudata.Dataset):
    def __init__(self,root,phase):
        ...
        self.imgs = imgs
        self.augment = InstanceAugmentation(p=0.2, phase=phase)
    ...
    def __getitem__(self, index):
        ...
        # random augmentation
        spacing = (1.0,1.0,1.0)
        #img_x, _, _ = self.augment(img_x, tree=None, spacing=spacing)
...
def validate(model, loss, data_iter, phase, debug=True, num_image_save=10):
    ...
    return loss_sum / n, acc_sum_loose / n_loose, acc_sum_strict / n_stric, val_all
...
def train_model(model, loss, optimizer, dataloaders, valloaders, output_file, num_epochs=20):
    ...
    for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                ...
                for ii in range(len(save_train_x)):
                    ...
                    for i,obj in enumerate(file_prefix.split("_")[0]):
                        ## 下面的i要加1 因为save_train_x是4维的，第一维是原图
                        if obj == "1":
        ...
        #val_all.extend(val)
        print(f"[Val{epoch}] average CELoss is {avg_val_loss:.3f}")
...
if __name__ == "__main__":
    ...
    output_file = "/home/yjz/Project/Auto-tracing/neuronet/neuronet_new_0519_crossing_threeUniteModality/neuronet/exps/exp004"
    batch_size = 64
    crossing_dataset = CrossingDataset("/home/yjz/Project/Auto-tracing/neuronet/neuronet_new_0519_crossing_threeUniteModality/neuronet/data/task001_allData", "train")
    dataloaders = tudata.DataLoader(crossing_dataset,batch_size=batch_size, shuffle=True, num_workers=1, drop_last=True)
    val_dataset = CrossingDataset("/home/yjz/Project/Auto-tracing/neuronet/neuronet_new_0519_crossing_threeUniteModality/neuronet/data/task001_allData" , "val")
    valloaders = tudata.DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=1, drop_last=True)
    ...
    model = train_model(net, loss, optimizer, dataloaders, valloaders, output_file, num_epochs=80)
